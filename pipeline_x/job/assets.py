...
@dagster.asset(deps=[ get_close_api_key ])
def get_close_data(get_close_api_key:Tuple[Dict, Dict]) ->  List[Dict]:
    yesterday = str(date.today() - timedelta(2))
    today = str(date.today() - timedelta(1))
    cf, d = get_close_api_key
    logger.info(cf)
    logger.info(d)
    api_key=d['SubTo']
...
@dagster.asset(deps=[ get_close_data ])
def input_snowflake_data(get_close_data:List[Dict]) ->  pd.DataFrame:
    # Assuming you have a function to load data into Snowflake
    leads=get_close_data
...
@job
def close_data_pipeline():
    input_snowflake_data(get_close_data(get_close_api_key()))